...
void init_world(char** field) {
    ...
    char** figure;
    // Добавляем фигуры на поле. Координаты выбраны просто из головы
    figure = get_frog();
...
int get_count_of_neighbors(char** field, int x0, int y0) {
    ...
    // Цикл обходит ячейку по кругу и считает количество соседей
...
char** get_letterL() {
    char** get_letterL = calloc_matrix(15, 15);
