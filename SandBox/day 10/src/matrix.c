...
int main() {
    ...
    if (read_type(&type, &rows, &cols)) {
        ...
        if (type == 2) {
            matrix = (int **)malloc(rows * sizeof(int*));  // Выделяем память под количество строк
            for (int i = 0; i < rows; i++)
                matrix[i] = (int *)malloc(
                    cols *
                    sizeof(
        ...
        if (type == 3) {
            matrix = (int **)calloc(rows, sizeof(int*));  // Выделяем память под количество строк
